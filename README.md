...
```bash
p2p-solution-sls$ sam build --use-container
```
...
```bash
p2p-solution-sls$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
p2p-solution-sls$ sam local start-api
p2p-solution-sls$ curl http://localhost:3000/
```
...
```bash
p2p-solution-sls$ sam logs -n HelloWorldFunction --stack-name p2p-solution-sls --tail
```
...
```bash
p2p-solution-sls$ pip install -r tests/requirements.txt --user
# unit test
p2p-solution-sls$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
p2p-solution-sls$ AWS_SAM_STACK_NAME="p2p-solution-sls" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "p2p-solution-sls"
```
